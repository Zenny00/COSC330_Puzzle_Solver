...
public class ArrowGridPuzzle implements Puzzle {
    ...
    public void setProblem(String input) {
        ...
        for (int j = 0; j < arrow_values.length; j++)
        {
            ...
            for (int i = 0; i < accepted_arrow_gird_values.length; i++)
                arrow_value = arrow_value.replace(accepted_arrow_grid[i], accepted_arrow_gird_values[i]);
    ...
    public String findSolution() {
        ...
        for (int i = 1; i < arrow_values.length; i++)
        {
            ...
            if (arrow_values[i].equals("r"))
                value++;
            else if (arrow_values[i].equals("l"))
                value--;
            else if (arrow_values[i].equals("u"))
                value += 3;
            else if (arrow_values[i].equals("d"))
                value -= 3;
        }
